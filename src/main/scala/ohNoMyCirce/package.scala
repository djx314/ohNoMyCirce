...
package object ohNoMyCirce {
  ...
  trait SlickFromApply[Case] {
    def apply[H, Data, E](rep: H)(implicit shape: Shape[_ <: ShapeLevel, H, Data, E]): Unit = macro OhNoMySlickMacros.OhNoMySlickMacrosImpl.impl[Case, Data, H, E]
  }

  def slickFrom[Case]: SlickFromApply[Case] = new SlickFromApply[Case] {}

  def slickSnippet[H, R, Data, O](t: H)(implicit shape: Shape[_ <: FlatShapeLevel, H, Data, O]): MappedProjection[net.scalax.ohNoMySlick.macros.OhNoMySlickMacros.tag.Aux[Data, R], Data] = macro OhNoMySlickMacros.SlickSnippetImpl.impl[H, Data, O]
